...
func (a *agent) runWait(epoch int) {
	// Misses due to over-approximation of go calls in main due to
	// re-encountering go calls in butterfly cycle.
	a.statusCh <- exitStatus(epoch) //@ blocks(g1), blocks(g2), fn
}
