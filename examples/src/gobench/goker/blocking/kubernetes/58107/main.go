...
func (q *Type) Get() {
	...
	q.cond.Wait() //@ blocks(G1), blocks(G2)
}
...
func (rq *ResourceQuotaController) worker(queue RateLimitingInterface, name string) func() {
	workFunc := func() bool {
		rq.workerLock.RLock() //@ blocks(G1), blocks(G2), fn
...
func (rq *ResourceQuotaController) Run() {
	go rq.worker(rq.queue, "G1")()             // G1 //@ go(g1)
	go rq.worker(rq.missingUsageQueue, "G2")() // G2 //@ go(g2)
}

func (rq *ResourceQuotaController) Sync() {
	for i := 0; i < 100000; i++ {
		rq.workerLock.Lock() //@ blocks(G3)
...
func startResourceQuotaController() {
	...
	go resourceQuotaController.Run()  //@ go(g0)
	go resourceQuotaController.Sync() //@ go(g3)
