...
func splitAndScatter(ctx context.Context, readyForImportCh chan bool) {
	for i := 0; i < chanSize+2; i++ {
		readyForImportCh <- (false || i != 0) //@ blocks, fn
		// TODO: Investigate this more.
		// I think the problem is that the buffer can be [0;6] at this control location.
		// And we have no path sensitivity for buffers (which would help here).
	}
}
