...
func (c *Client) Close() {
	// Spurious cycle due to context creates fp.
	// c.cancel returns to main instead of here.
	c.mu.Lock() //@ releases(main), fp
	defer c.mu.Unlock()
	if c.cancel == nil {
		return
	}
	...
	c.cancel = nil
	c.mu.Unlock()
	// Goat: Spurious cycle due to c.cancel makes information too imprecise at this point.
	c.mu.Lock() //@ blocks(main), fn
}
