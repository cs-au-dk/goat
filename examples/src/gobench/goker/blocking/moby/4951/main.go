...
func (devices *DeviceSet) DeleteDevice(hash string) {
	...
	// We cannot detect this bug because we cannot prove that
	// the DeviceSet pointers are equal.
	// Stronger map model is needed.
	info.lock.Lock() //@ blocks, fn
