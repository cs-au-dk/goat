...
func (out chanOutput) WriteProgress(p Progress) error {
	// This is not described in the bug but is a true positive.
	// If we do two calls to WriteProgress but only do one receive,
...
func (t *Transfer) Watch(progressOutput Output) *Watcher {
	...
	go func() { // G2
		...
		for {
			...
			// Imprecise loop over xrefs in main leaves releaseChan open
			// (Data-flow is not merged after the loop because main blocks
			// if it does actually enter the loop).
			select { //@ releases, fp
			case <-w.signalChan:
			case <-w.releaseChan:
				done = true
