...
/* GoLive (O):
	I think the reason why we cannot detect the bugs is because
	the Mutex pointers get mixed in the requests slice.
	Therefore we cannot rule out the possibility that the "right"
	lock is unlocked (wrt. causing a deadlock otherwise) in `unlock`.
*/
